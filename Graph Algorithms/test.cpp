...
void solve(){
    ...
    rep(i, N){
        if(dp[i] == 1){
            flow.make_edge(501, i, 1);
        }
        if(dp[i] == maxi){
            flow.make_edge(i, 502, 1);
        }
        for(int j = 1; j < i; j++){
            if(dp[j] + 1 == dp[i] and arr[j] <= arr[i]){
                flow.make_edge(j, i, 1);
            } 
        }
    }
    cout << flow.max_flow(501, 502) << "\n";
    flow.reset();
    rep(i, N){
        if(dp[i] == 1){
            if(i != 1) flow.make_edge(501, i, 1);
            else flow.make_edge(501, i, MAXF);
        }
        if(dp[i] == maxi){
           if(i != N) flow.make_edge(i, 502, 1);
           else flow.make_edge(i, 502, MAXF);
        }
        for(int j = 1; j < i; j++){
            if(dp[j] + 1 == dp[i] and arr[j] <= arr[i]){
                flow.make_edge(j, i, 1);
